...
class UnluckyVassal {
    public void printReportForKing(List<String> pollResults) {
        ...
        recursivePrint(king, 0);
    }

    /** Рекурсивно обходит дерево, и печатает его**/
    private void recursivePrint(Person person, int level) {
        ...
        if (person.getServants() != null) {
            for (Person servant : person.getServants()) {
                recursivePrint(servant, level + 1);
            }
        }
    }

    //Шаблон для строки
    private static final Pattern RECORD = Pattern.compile("^(([а-яА-Я ]*): )?(([а-яА-Я ]*?), )*([а-яА-Я ]*)$");
    ...
    private void parseRecord(String record, Map<String, Person> personByName) {
        ...
        if (!RECORD.matcher(record).find()) throw new RuntimeException("Строка \"" + record + "\" не соответствует шаблону");
